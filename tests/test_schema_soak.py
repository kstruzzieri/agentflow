...
class SchemaSoakCheckerTests(SoakHarness, unittest.TestCase):
    ...
    def test_rejects_unresolvable_candidate_without_traceback(self) -> None:
        ...
        self.assertEqual(result.returncode, 1)
        self.assertIn("schema soak check failed:", result.stderr)
        # Issue #36: name the field, the SHA, and the squash-merge cause instead
        # of surfacing `fatal: Needed a single revision`.
        self.assertIn("candidate_commit", result.stderr)
        self.assertIn("does not exist in this repository", result.stderr)
...
class OneZeroTransitionTests(SoakHarness, unittest.TestCase):
    ...
    def test_transition_may_regenerate_the_ci_proof_bundle(self) -> None:
        # verify-run carries no cross-major promise, so the live CI bundle is
        # rebuilt by the 1.0 code inside the recorded transition.
        self._elapsed_manifest()
        self._transition()
        bundle = self.root / "tests/fixtures/proof-bundle"
        (bundle / "fixture.txt").write_text(
            "regenerated by the 1.0 code\n", encoding="utf-8"
        )
        (bundle / "regenerated.json").write_text("{}\n", encoding="utf-8")
        self._record_transition()
    ...
    def test_transition_may_drop_stale_ci_proof_bundle_files(self) -> None:
        self._elapsed_manifest()
        self._transition()
        bundle = self.root / "tests/fixtures/proof-bundle"
    ...
    def test_proof_bundle_regeneration_requires_the_recorded_transition(self) -> None:
        self._elapsed_manifest()
        (self.root / "tests/fixtures/proof-bundle/fixture.txt").write_text(
            "tampered\n", encoding="utf-8"
        )
        ...
        self.assertEqual(result.returncode, 1)
        self.assertIn("tests/fixtures/proof-bundle/fixture.txt", result.stderr)
